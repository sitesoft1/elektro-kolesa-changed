...
class SeoPro {
    ...
    private function getPath2($categories, $category_id, $current_path = []) {
        ...
        if($parent_id > 0) {
            $new_path =  array_merge ([$parent_id] , $current_path);
            $path =  $this->getPath2($categories, $parent_id, $new_path);
        }
	...
	private function initHelpers() {
		...
		if($this->config->get('config_seo_url_cache')){		
			...
			$this->cat_tree2 = $this->cache->get('seopro.cat_tree2');
		}
		
		if(!$this->cat_tree || empty($this->cat_tree) || !$this->cat_tree2 || empty($this->cat_tree2)) {
	...
	private function getCategoryByProduct($product_id) {
		...
		$category_id = $this->getPathByCategory2($query->num_rows ? (int)$query->row['category_id'] : 0);
    ...
    private function getPathByCategory2($category_id) {
        
        $path = '';
        
        if ((int)$category_id < 1 && !isset($this->cat_tree2[$category_id]))
            return false;
        
        if (!empty($this->cat_tree2[$category_id]['path']) && is_array($this->cat_tree2[$category_id]['path'])) {
            $path = implode('_', $this->cat_tree2[$category_id]['path']);
        }
	...
	public function __destruct() {	
		...
		if ($this->config->get('config_seo_url_cache')){	
			...
			$this->cache->set('seopro.cat_tree2', $this->cat_tree2);
