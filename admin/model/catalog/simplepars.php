  ...
  if(empty($pars_url['links'])){
    ...
    $this->answjs('finish','Парсинг закончился, ссылок больше нет﻿',$answ);
  	...
  	foreach($datas as $key => $data){
  		...
  		#помечаем ссылку как отсканированная
    	$this->db->query("UPDATE ".DB_PREFIX."pars_link SET scan=0, error='".$curl_error['http_code']."' WHERE link='".$this->db->escape($data['url'])."' AND dn_id=".$dn_id);
	...
	foreach($imgs_chunk as $chunk){
		...
		foreach($data_img as $key => $img){
			...
			if(empty($exec['extension'])){
				...
				$ext = (preg_match('#(^jpeg)|(^jpg)|(^png)|(^jpe)|(^gif)|(^bmp)#i', $exec['extension'])) ? '.'.$exec['extension'] : '.jpg';
			}
			#$this->wtfarrey($exec);
...
public function getProductStatus($data, $setting){
	...
	#$this->wtfarrey($data);
	#$this->wtfarrey($setting['r_quant']);
	#$this->wtfarrey($data['quant']);
	return $status;
}
...
public function controlOption($data, $setting, $langs, $pr_id, $dn_id){
  ...
  // 1 - Создать/Добавить/Обновить
	////////////////////////////////////////////////////
	#$this->wtfarrey($data);
	if ($setting['r_opt'] == 1 || $setting['r_opt'] == 2){
...
//Унция удаленеия опций которые не обновились.
public function delOptNotBeenUpdate($del_arrey, $pr_id, $setting, $dn_id){
	...
	if(!empty($del_arrey['opt'])){ $sql_del_opt .= " AND option_id not in (".$del_arrey['opt'].")";	}
	$this->db->query($sql_del_opt);

	//Выполняем удаление всех значений опций которых нет в обновлении.
	$sql_del_value = "DELETE FROM ".DB_PREFIX."product_option_value WHERE product_id = ".$pr_id;
	if(!empty($del_arrey['opt'])){ $sql_del_value .= " AND option_value_id not in (".$del_arrey['value'].")";	}
	$this->db->query($sql_del_value);
	#$this->wtfarrey($sql_del_opt);
	#$this->wtfarrey($sql_del_value);
}
  		...
  		if($curl_error['error']){ 
			...
			if($setting['action'] == 1){
				...
				if($permit['add']['permit'] == 1){
					$this->addProduct($form, $link, $setting, $dn_id, $browser);
				}else{
...
public function dwImgToDesc($desc, $url, $des_dir, $img_name, $under, $dn_id, $browser){
	...
	if(!empty($imgs_tmp)){
		...
		foreach($imgs_tmp[0] as $key_img => $var){
			#$this->wtfarrey($var);
			//Удаляем лишние
			$var = preg_replace('#\{img\}(.*?)src="#u', '', $var);
...
public function preparinDataToStore($data, $dn_id){
	...
	$form['sku'] = substr(trim( $this->madeLogicalMathem(str_replace('{!na!}','',$form['sku']), 'str')), 0, 64);
	...
	#$this->wtfarrey($form['quant']);
	$form['quant'] = $this->madeLogicalMathem($form['quant'], 'int');
	#$this->wtfarrey('После логики'.$form['quant']);
	...
	$form['layout_pr'] = (int)str_replace('{!na!}','',$form['layout_pr']);
	$form['layout_cat'] = (int)str_replace('{!na!}','',$form['layout_cat']);

	$form['tags'] = trim(str_replace('{!na!}','',$form['tags']));
...
public function addProduct($data, $link, $setting, $dn_id, $browser){
	...
	if($permit == 1){
		...
		if($pr_id){
			...
			// 2 - Не создавать новые | Обновлять категории в товаре
			// 3 - Создавать новые | Добавлять в товар эти категории
			///////////////////////////////////////
			$data['cats_id'][0] = 0;
			if($setting['r_cat'] == 0){
			...
			}elseif($setting['r_cat'] == 1 || $setting['r_cat'] == 3){
...
public function updateProduct($data, $link, $setting, $dn_id, $pr_id=0, $browser){
	...
	if($pr_id > 0){
		...
		//не раскоментирыват, это для примера пользователям.
		...
		if(!empty($set_pr_desc)){
			...
			foreach ($langs as $lang) {
				...
				if($sql_desc->num_rows > 0) {
					...
					$sql = "INSERT INTO " . DB_PREFIX . "product_description SET ".$set_pr_desc;
					//Запрос
					$this->db->query($sql);
				}
			}
	...
	while($off == 0){
		...
		if($task['task'] == 1){
			...
			if($links->num_rows == 0){ 
		  	...
		  	foreach($datas as $key => $data){
		  		...
		  		$curl_error = $this->sentLogMultiCurl($data ,$dn_id);
					...
					#помечаем ссылку как отсканированная
		    	$this->db->query("UPDATE ". DB_PREFIX ."pars_link SET `scan_cron`=0, error='".$curl_error['http_code']."' WHERE link='".$this->db->escape($data['url'])."' AND dn_id=".$dn_id);
		  ...
		  $browser['cache_page'] = $task['cache_page'];
			...
			//проверка отключена всвязи с больщим размером файла, модулю нужно максимальное время для обработки этих файлов.
		  #$this->cronChackTimeout();
			//Обрабатываем данные с мульти запроса. 
	...
	if($mark == 'log_curl'){
		...
		if($data['errno'] == 0){
			...
			if($data['http_code']==200){
	  		...
	  		$text .= 'ОШИБКА '.$text_proxy .' | Forbidden, доступ запрещен. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  		...
	  		$text .= 'ОШИБКА '.$text_proxy .' | Locked — целевой ресурс из запроса заблокирован от применения к нему указанного метода. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}elseif($data['http_code']==429){
	  		$text .= 'ОШИБКА '.$text_proxy .' | Страница недоступна, слишком много запросов за короткое время. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}elseif($data['http_code']==426){
	  		$text .= 'ОШИБКА '.$text_proxy .' | Сайт отказывается выполнять запрос с использованием текущего протокола. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}elseif($data['http_code']==500){
	  		$text .= 'ОШИБКА '.$text_proxy .' | Сайт донор не может обработать запрос (Internal Server Error, внутренняя ошибка сервера). Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}elseif($data['http_code']==502){
	  		$text .= 'ОШИБКА '.$text_proxy .' | Донор получил не верны ответ от вышестоящего сервера. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}elseif($data['http_code']==503){
	  		$text .= 'ОШИБКА '.$text_proxy .' | Сайт донор недоступен на данный момент. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}elseif($data['http_code']==504){
	  		$text .= 'ОШИБКА '.$text_proxy .' | Внутренняя ошибка донора (Gateway Timeout), не получен своевременный ответ. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}elseif($data['http_code']==505){
	  		$text .= 'ОШИБКА '.$text_proxy .' | HTTP-версия, используемая в запроcе, не поддерживается донором. Ответ сервера ['.$data['http_code'].'] Ссылка | '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
	  	}else{
  		...
  		$text .= 'ОШИБКА запроса '.$text_proxy .' | Код ответа = '.$data['errno'].' | Текст ответа = '.$data['errmsg'].' | Ссылка - '.htmlspecialchars_decode(trim($data['url'])).PHP_EOL;
...
public function xmlShowPieceCode($data, $dn_id){
	...
	if(!empty($data['cat_work'])){
			...
			$show_code .= '!############################## Визульное прдеставления категорий. Только для поред простмра!!! ##############################!'.PHP_EOL.PHP_EOL;
			if(!empty($cat_forms)){
			...
			$show_code .= PHP_EOL.'!######################################### Конец визуельного представления категорий #########################################!'.PHP_EOL.PHP_EOL.PHP_EOL.PHP_EOL;
...
public function xmlCutsAnswerFromCurl($urls, $dn_id){
	...
	#$this->wtfarrey(substr($code[$urls[0]]['content'], 0, 10000000));
...
public function controlParsToXml($dn_id){
  ...
  $pars_xml = $this->db->query("SELECT * FROM ".DB_PREFIX."pars_xml WHERE `dn_id`=".(int)$dn_id);
  $pars_xml = $pars_xml->row;
...
public function xmlControlSplitAndCut($pars_xml, $url, $setting, $dn_id){
	...
	while( ($iterator->current() !== null) || !empty($text_end)){
		...
		#if($pr_num < 10){
			//Если найден текст начала то начинаем писать. 
			#$line = mb_convert_encoding($line, "UTF-8", $chrs);
	...
	//записываем в логи.
	$this->log('cutNewXml', $log['culum'] = $pr_num, $dn_id);
...
public function curlImg($urls,  $browser=[], $dn_id){
  ...
  foreach($urls as $i => $url) {
   	$curl_array[$i] = curl_init($url);
	  //настраиваем браузер
	  #curl_setopt($curl_array[$i], CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2);
	  curl_setopt($curl_array[$i], CURLOPT_RETURNTRANSFER, 1);                            // возвращает веб-страницу
	  ...
	  curl_setopt($curl_array[$i], CURLOPT_MAXREDIRS, 0);                                // останавливаться после 10-ого редиректа
	  curl_setopt($curl_array[$i], CURLOPT_ENCODING, "");                                 // обрабатывает все кодировки 
...
public function urlEncoding($url){
    ...
    $restored = strtr($restored, ['%3D'=>'=','%2C'=>',','%2B'=>'+','&amp;'=>'&']);// Ибо rawurlencode заменяет равенство '=' на '%3D'
